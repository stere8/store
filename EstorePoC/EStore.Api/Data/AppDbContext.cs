...
{
    // Tenant scoping (set by middleware)
    ...
    protected override void OnModelCreating(ModelBuilder m)
    {
        ...
        m.Entity<ShoppingCart>(e =>
        {
            ...
            // Ensure one active cart per tenant+customer (enforced by code path; index helps)
        ...
        m.Entity<Review>(e =>
        {
            ...
            // One review per (tenant, product, customer)
            e.HasIndex(x => new { x.TenantId, x.ProductId, x.CustomerId }).IsUnique();
        });

        // ---------------- Global query filters (multi-tenant) ----------------
        // Important: Program.cs sets CurrentTenantId on the DbContext per request.
        ...
        // Optional: seed minimal tenants for demo (safe for InMemory)
